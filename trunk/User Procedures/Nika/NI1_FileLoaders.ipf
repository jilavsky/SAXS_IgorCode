...
Function NI1A_WriteHdf52DCanSASData(AppendToNexusFile, Hdf5FileName, Iwv, [dIwv, Qwv, Mask, Qx, Qy,AzimAngles,UnbinnedQx,UnbinnedQy])
		...
		//ﾊﾊ@NX_class = "NXentry"			<-- NeXus requires this
		//ﾊﾊ@canSAS_class = "SASentry"		<-- this is between you and me, for now. Perhaps optional?
		//
		//ﾊﾊ/sasdata01						<-- HDF5 group
		//ﾊﾊﾊﾊ@NX_class = "NXdata"			<-- NeXus requires this
		//ﾊﾊﾊﾊ@canSAS_class = "SASdata"		<-- this is between you and me, for now.
		//ﾊﾊﾊﾊ@canSAS_version = "0.1"		<-- this is between you and me, for now.
		//	ﾊ	@I_axes = "Q,Q"				<-- canSAS 2012 agreed model
		//	ﾊ	@Q_indices = "0,1"			<-- canSAS 2012 agreed model
		//	ﾊ	@Mask_indices = "0,1"			<-- canSAS 2012 agreed model
		//
		//	ﾊI: float[M,N]					<-- HDF5 dataset, the intensity array
		//	ﾊﾊﾊﾊ@uncertainty="Idev"		<-- canSAS 2012 agreed model
		//	ﾊﾊﾊﾊ@signal=1					<-- NeXus requires this
		//		   @axes="Q"					<-- NeXus suggests this is identified
		//	ﾊIdev: float[M,N]				<-- HDF5 dataset, the intensity array <<<<optional
		//	ﾊQ: float[M,N]					<-- HDF5 dataset, |Q| at each pixel  <<<<optional
		//or
		//	ﾊ	@I_axes = "Qx,Qy"				<-- canSAS 2012 agreed model
		//	Qx: float[M,N] ﾊﾊ<-- Qx at each pixel
		//	Qy: float[M,N] ﾊﾊ<-- Qy at each pixel
		//	Qz: float[M,N] ﾊﾊ<-- Qz at each pixel (might be all zero)
