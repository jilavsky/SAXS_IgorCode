...
Function/S NI2_LoadGeneralHDFFile(CalledFrom, fileName, PathName)
	string CalledFrom,  fileName, PathName
	
	abort "General HDF5 load is broken due to Igor 9 code modifications. Remind author to fix it (and send example data set, please)"
	//CalledFrom ... "Nika" or "Irena"
	...
	variable i		//what if user wants more groups???
	For(i=0;i<numpnts(GroupNames);i+=1)
		HDF5LoadGroup /O /R /T /IMAG=1 :, locFileID, GroupNames[i]
	endfor
	string/g GroupPathsLoaded, dataFolderPathsLoaded, ObjectPathsLoaded
	
	GroupPathsLoaded  		= 	S_groupPaths
	dataFolderPathsLoaded	=	S_dataFolderPaths
	ObjectPathsLoaded 		=	 S_objectPaths
	HDF5CloseFile locFileID
 ...
 Function NI2_ParseNikaData()		//here we parse loaded groups for Nika. Notre - Nika expects ONE 2D image from the hdf file... 
	//need to create ONE "LoadedWave" in the Nika folder from what we have here... 
	//here is what is available:
	setDataFolder root:Packages:TempHDFLoad
	SVAR GroupPathsLoaded = root:Packages:TempHDFLoad:GroupPathsLoaded
	SVAR dataFolderPathsLoaded=root:Packages:TempHDFLoad:dataFolderPathsLoaded
	SVAR ObjectPathsLoaded=root:Packages:TempHDFLoad:ObjectPathsLoaded
	
	//for now assume only one group loaded for Nika.... If we will need more, we'll have to kwnowhat users want
	//	string PathToData=stringfromList(0,GroupPathsLoaded,";")
	//	PathToData = stringfromList(ItemsinList(PathToData, "/")-1, PathToData, "/")
//	if(DataFolderExists(PathToData) )		//does the folder exist as expected?
//		setDataFolder $(PathToData)
//
//
//	else
//		Abort "Parsing failed, send example of data to ilavsky@aps.anl.gov"
//	endif
	variable i
	For(i=0;i<itemsInList(ObjectPathsLoaded,";");i+=1)
		wave tempWv=$(stringFromList(i,ObjectPathsLoaded,";"))
		if(dimsize(tempWv,1)>0 && dimsize(tempWv,2)==0)	//thsi is 2 D wave
			//do something with this wave
			
			return 1
		endif	
	endfor
	Abort "Parsing Nika data filed, not 2D wave found"
...
static Function CreateHDF5BrowserGlobals(newBrowserName)
	...
	String savedDataFolder = NI2_SetBrowserDataFolder(newBrowserName)
...
End
//
Function/S NI2_SetBrowserDataFolder(browserName)		// Pass "" for browserName to set to master browser folder.
	String browserName
...
End
//
//static Function SetNI2HDF5BrowserData(browserName, bd)
//	String browserName
//	STRUCT NI2HDF5BrowserData &bd
//	
//	String savedDataFolder = NI2_SetBrowserDataFolder(browserName)
//	
...
//End
//
//Function/S NI2_GetGroupHierarchy(fileID, startPath, level, mode)
//	Variable fileID
//	String startPath
//	Variable level
...
//	endif
//	hierarchy = rootName + ";" + NI2_GetGroupHierarchy(bd.fileID, "/", 1, mode)
//	
//	return hierarchy
//End
//
...
//End
//
//Function/S NI2_HDF5GetObjectFullPath(groupPath, objectName)
...
//End
//
//Function/S NI2_GetTextPreviewString(tw)
...
//End
//
//Function/S NI2_GetNumericPreviewString(w)
...
//End
//
//Function/S NI2_GetPreviewString(locationID, objectType, di, fullPath, attributeName)
//	Variable locationID
...
//				endif
//				
//				value = NI2_GetNumericPreviewString(tempNumericAttributeWave)
//			endif
...
//				endfor
//				
//				value = NI2_GetTextPreviewString(references)
//			endif
...
//					Redimension/N=(numElements)/E=1 tempTextAttributeWave
//				endif
//				value = NI2_GetTextPreviewString(tempTextAttributeWave)
//			endif
//			KillWaves/Z tempTextAttributeWave
//			break				
//			
...
//			String fullPath = NI2_HDF5GetObjectFullPath(bd.groupPath, dataset)
//#if(IgorVersion()<9)
//			STRUCT NI2_HDF5DataInfo di
...
//			err = HDF5DatasetInfo(bd.fileID, fullPath, 0, di)
//#endif		
//			
//			
//			Variable rank = di.ndims
...
//
//			String preview = NI2_GetPreviewString(bd.fileID, 2, di, fullPath, "")
...
//
//			String preview = NI2_GetPreviewString(bd.fileID, 1, di, groupPath, attributeName)
...
//
//			String preview = NI2_GetPreviewString(bd.fileID, 2, di, datasetPath, attributeName)
...
//End
//
//Function/S NI2_SelectedGroupName(bd)
//	STRUCT NI2HDF5BrowserData &bd
//	
...
//End
//
//Function/S NI2_SelectedGroupPath(bd)
//	STRUCT NI2HDF5BrowserData &bd
//	
//	String groupPath = bd.groupPath
//
...
//End
//
//Function/S NI2_SelectedDatasetName(bd)
//	STRUCT NI2HDF5BrowserData &bd
//	
...
//End
//
//Function/S NI2_SelectedDatasetPath(bd)
//	STRUCT NI2HDF5BrowserData &bd
//
//	String datasetName = NI2_SelectedDatasetName(bd)
//	if (strlen(datasetName) == 0)
...
//End
//
//Function/S NI2_SelectedAttributeName(bd, isGroupAttribute)
//	STRUCT NI2HDF5BrowserData &bd
...
//End
//
//Function/S NI2_SelectedAttributePath(bd, isGroupAttribute)
//	STRUCT NI2HDF5BrowserData &bd
//	Variable isGroupAttribute
//
//	String attributeName = NI2_SelectedAttributeName(bd, isGroupAttribute)
//	if (strlen(attributeName) == 0)
//		return ""						// Nothing is selected
//	endif
//	
//	String path
//	if (isGroupAttribute)
//		path = NI2_SelectedGroupPath(bd)
//	else
//		path = NI2_SelectedDatasetPath(bd)
//	endif
//	
//	path += "/" + attributeName
//
...
//End
//
//StrConstant NI2_kLoadAllMembersString = "_Load_All_Members_"
//
...
//
//	hideMembers = 1
//	memberList = NI2_kLoadAllMembersString + ";"
//	
//	if (bd.fileID != 0)				// File is open for this browser?
//		String fullPath
//		
//		fullPath = NI2_SelectedDatasetPath(bd)
//		if (strlen(fullPath) > 0)
//			STRUCT NI2_HDF5DataInfo di
//			NI2_InitHDF5DataInfo(di)
//			HDF5DatasetInfo(bd.fileID, fullPath, 0, di)
...
//				STRUCT NI2_HDF5DatatypeInfo dti
//				NI2_InitHDF5DatatypeInfo(dti)
...
//End
//
//Function NI2_HDF5GetCompLoadInfo(bd, isCompound, compMode, memberName)
//	STRUCT NI2HDF5BrowserData &bd
...
//		isCompound = 1
//		memberName = S_value
//		if (CmpStr(memberName, NI2_kLoadAllMembersString) == 0)
//			memberName = ""
//		endif
//	endif
//
...
//End
//
//Function NI2_HDF5GetTranspose2DSetting(browserName)
//	String browserName
//	
//	Variable result
...
//	
//	STRUCT NI2HDF5BrowserData bd
//	SetNI2HDF5BrowserData(browserName, bd)
//	
//	NI2_HDF5DisplaySelectedDataset(bd)
//End
//
...
//		Button CreateFile, win=$bd.browserName, disable=2		// Enable Create
...
//		
//		String groupName = NI2_SelectedGroupName(bd)
...
//		//overwrite the code above...
//		Button LoadGroup, win=$bd.browserName, disable=2	// Disable Load Dataset
//		Button LoadDataset, win=$bd.browserName, disable=2	// Disable Load Dataset
//	endif
...
//	DrawFilePath(bd)
//	
//	bd.readOnly = isCreate ? 0 : NI2_HDF5GetReadOnlySetting(browserName)
//
//	bd.groupPath = "/"
//	DrawGroupPath(bd)
//
...
//	FillLists(bd)
//	
//	NI2_UpdateAfterGroupSelected(bd, "/")
//	
//	String datasetName = NI2_SelectedDatasetName(bd)
//	if (strlen(datasetName) > 0)
//		NI2_SelectDataset(bd, datasetName)
//	endif
//End
//
...
//	STRUCT NI2HDF5BrowserData bd
//	
//	if(stringmatch(ctrlName,"AddGrouptoList"))
//		
//		browserName = NI2_HDF5GetTopBrowserName()
//	
//		SetNI2HDF5BrowserData(browserName, bd)
//		
//		Wave/T DataSetNames=$("root:Packages:"+NI2_GetHDF5LoaderLocString()+":DataSetNames")
//		Wave/T GroupNames=$("root:Packages:"+NI2_GetHDF5LoaderLocString()+":GroupNames")
//		SVAR BrowserNameFldr=$("root:Packages:"+NI2_GetHDF5LoaderLocString()+":BrowserNameFldr")
//		Redimension/N=(numpnts(DataSetNames)+1) GroupNames
//		GroupNames[numpnts(GroupNames)-1]=NI2_SelectedGroupPath(bd)
//		BrowserNameFldr = NI2_HDF5GetTopBrowserName()
//	elseif(stringmatch(ctrlName,"ClearDatasetList"))
//		browserName = NI2_HDF5GetTopBrowserName()	
//		SetNI2HDF5BrowserData(browserName, bd)		
//		Wave/T DataSetNames=$("root:Packages:"+NI2_GetHDF5LoaderLocString()+":DataSetNames")
//		Wave/T GroupNames=$("root:Packages:"+NI2_GetHDF5LoaderLocString()+":GroupNames")
//		SVAR BrowserNameFldr=$("root:Packages:"+NI2_GetHDF5LoaderLocString()+":BrowserNameFldr")
//		Redimension/N=(0) DataSetNames, GroupNames
////		DataSetNames[numpnts(DataSetNames)-1]=NI2_SelectedDatasetPath(bd)
//		BrowserNameFldr = NI2_HDF5GetTopBrowserName()		
//	else		//Old LoadDataset button...
//	
//		browserName = NI2_HDF5GetTopBrowserName()
//	
//		SetNI2HDF5BrowserData(browserName, bd)
...
//		String memberName
//		Variable compMode
//		NI2_HDF5GetCompLoadInfo(bd, isCompound, compMode, memberName)
//	
...
//		if (!isCompound)
//			String savedDataFolder = NI2_SetBrowserDataFolder("")			// tempClassAttribute goes in master HDF5Browser data folder
...
//		else
//			Variable transpose2D = NI2_HDF5GetTranspose2DSetting(bd.browserName)
...
//	SetNI2HDF5BrowserData(browserName, bd)
//	
//	String groupPath = NI2_SelectedGroupPath(bd)
//
...
//			// This detects if the file is no longer open, such as if you save the experiment, quit Igor and then reopen the experiment.
//			if (NI2_FileWasUnexpectedlyClosed(bd))
...
//
//		case "resize":
//			NI2_HDF5ResizeBrowser(browserName)
//			break
//			
...
//			if (strlen(NI2_HDF5GetIndxBrowserName(1)) == 0)
...
//	else
//		String fullPath
//		fullPath = NI2_HDF5GetObjectFullPath(bd.groupPath, datasetName)
//		STRUCT NI2_HDF5DataInfo di
//		NI2_InitHDF5DataInfo(di)			// Set input fields.
//		HDF5DatasetInfo(bd.fileID, fullPath, 0, di)
//		// Print s
...
//			NI2_UpdateAfterGroupSelected(bd, fullGroupPath)
//			// Printf "Row=%d, column=%d, path=%s\r", s.row, s.col, fullGroupPath
//			if (NI2_HDF5BrowserDumpIsVisible())
//				NI2_HDF5DisplayDumpOfSelGroup(bd)
//			endif
//			break	
//	endswitch
//
...
//			NI2_HDF5DisplaySelAttribute(bd,  1)				// Update various windows if they are displayed
//			break	
//	endswitch
//
//	return result
//End
//
//Function NI2_HandleDatasetDoubleClick(s, bd)
//	STRUCT WMListboxAction &s
//	STRUCT NI2HDF5BrowserData &bd
//
//#if Exists("HDF5OpenFile")	
...
//	switch(s.eventCode)
//		case 3:						// Double-click
//			NI2_HandleDatasetDoubleClick(s, bd)
//			break;
//			
//		case 4:						// Cell selection
//			String name = bd.datasetsList[s.row][0]
//			NI2_SelectDataset(bd, name)
//			// Printf "Row=%d, column=%d, name=%s\r", s.row, s.col, name
//			NI2_HDF5DisplaySelectedDataset(bd)					// Update various windows if they are displayed
//			break	
//	endswitch
//	
//	return result
...
//		case 4:						// Cell selection
//			// Printf "Row=%d, column=%d\r", s.row, s.col
//			NI2_HDF5DisplaySelAttribute(bd,  0)				// Update various windows if they are displayed
//			break	
//	endswitch
//	
//	return result
...
//	else
//		NI2_HDF5CreateDumpWindow()						// Create if it does not exist.
...
//
//	SetVariable HyperSelectionWave,pos={140,155},size={340,16},title="Hyper Selection Wave:"
...
//	
//	Button CreateFile,pos={15,8},size={125,20},proc=NikaHDF5Browser#CreateFileButtonProc,title="Create HDF5 File", disable=1
//	Button OpenFile,pos={159,8},size={125,20},proc=NikaHDF5Browser#OpenFileButtonProc,title="Open HDF5 File"
//	Button CloseFile,pos={296,8},size={125,20},proc=NikaHDF5Browser#CloseFileButtonProc,title="Close HDF5 File"
//	Button Help,pos={435,8},size={50,20},proc=NikaHDF5Browser#HelpButtonProc,title="Help"
//	CheckBox ReadOnly,pos={186,32},size={68,14},title="Read Only",proc=HDF5BrowserPrefCheckboxProc,value=readOnly
//
//	// Start Preview
//
//	Button Graph,pos={556,27},size={90,20},proc=NikaHDF5Browser#GraphButtonProc,title="Show Graph"
//	Button Graph help={"Shows or hides a graph which displays the last dataset or attribute that you selected."}
//
//	Button Table,pos={672,27},size={90,20},proc=NikaHDF5Browser#TableButtonProc,title="Show Table"
//	Button Table help={"Shows or hides a table which displays the last dataset or attribute that you selected."}
//
//	Button Dump,pos={556,59},size={90,20},proc=NikaHDF5Browser#DumpButtonProc,title="Show Dump"
...
//
//	ListBox GroupsList,pos={15,250},size={306,170}, mode=2, proc=NikaHDF5Browser#ListBoxActionProc
//	ListBox GroupsList,fSize=14
//
//	Button LoadGroup,pos={80,224},size={100,20},proc=NikaHDF5Browser#LoadGroupButtonProc,title="Load Group"
//	Button LoadGroup,help={"Loads the currently selected group into the current data folder."}
//
//	Button SaveDataFolder,pos={194,224},size={120,20},proc=NikaHDF5Browser#SaveDataFolderButtonProc,title="Save Data Folder"
...
//
//	ListBox GroupAttributesList,pos={15,455},size={306,109}, mode=2, proc=NikaHDF5Browser#ListBoxActionProc
...
//
//	ListBox DatasetsList,pos={341,250},size={459,170}, mode=2, proc=NikaHDF5Browser#ListBoxActionProc
...
//
//	ListBox DatasetAttributesList,pos={341,455},size={459,109}, mode=2, proc=NikaHDF5Browser#ListBoxActionProc
...
//
//
//	Button LoadDataset,pos={415,224},size={100,20},proc=NikaHDF5Browser#LoadDatasetButtonProc,title="Load Dataset"
//	Button LoadDataset,help={"Loads the currently selected dataset into the current data folder."}
//
//	Button SaveWaves,pos={529,224},size={100,20},proc=NikaHDF5Browser#SaveWavesButtonProc,title="Save Waves"
...
//	//Nika stuff
////	Wave/O/N=0/T DataSetNames, GroupNames		//these are pointes to waht user wants to get into Igor...
//	TitleBox NikaDataSetsTitle,pos={801,10},size={62,16},disable=2,title="Selected Datasets"
//	TitleBox NikaDataSetsTitle,fSize=14,frame=0,fStyle=1
//
//	string tempWvName="root:Packages:"+NI2_GetHDF5LoaderLocString()+":GroupNames"
//	ListBox NikaGroupList,pos={755,35},size={220,200}, mode=2, proc=NikaHDF5Browser#ListBoxActionProc
//	ListBox NikaGroupList, widths={175,40,80,120,1000}, userColumnResize= 1	// userColumnResize requires Igor Pro 5.02.
//	ListBox NikaGroupList,fSize=14, listwave=$(tempWvName)
//
//	Button AddGrouptoList,pos={600,200},size={150,20},proc=NikaHDF5Browser#LoadDatasetButtonProc,title="Add Group to Nika List"
//	Button AddGrouptoList,help={"Loads the currently selected Group into the current data folder."}
//	Button ClearDatasetList,pos={600,200},size={150,20},proc=NikaHDF5Browser#LoadDatasetButtonProc,title="Clear Nika List"
//	Button ClearDatasetList,help={"Clears Nika list of selected datasets"}
//
//
...
//	
//	NI2_HDF5ResizeBrowser(browserName)		// Needed because we used preferred browser size.
//	
//	SetWindow kwTopWin,hook=NI2_HDF5BrowserPanelHook
//EndMacro
//
//
//static Function/S NI2_GetHDF5LoaderLocString()
//	
//	string CurPanelName = WinName(0,64)
//	if(stringmatch(CurPanelName[0,3],"Nika"))
//		return "NikaHDF5Loader"
//	elseif(stringmatch(CurPanelName[0,4],"Irena"))
//		return "IrenaHDF5Loader"
//	else
//		return ""
//	endif
//end
//
//Function NI2_HDF5BrowserPrefCheckboxProc(ctrlName,checked) : CheckBoxControl
//	String ctrlName
//	Variable checked
//	
//	STRUCT NI2_HDF5BrowserPrefs prefs
//	
//	NI2_HDF5BrowserLoadPackagePrefs(prefs)
//	strswitch(ctrlName)
//		case "ReadOnly":
...
//		
//	endswitch
//	NI2_HDF5BrowserSavePackagePrefs(prefs)
//End
//
...
//End
//
//Function/S NI2_HDF5GetIndxBrowserName(index)
//	Variable index
//	
//	if (index < 0)
//		return ""				// Bad index
...
//End
//
//Function/S NI2_HDF5GetTopBrowserName()
//	String browserName = NI2_HDF5GetIndxBrowserName(0)
//	return browserName
//End
//
//Function NI2_HDF5AreAnyBrowsersOpen()
//	String browserName = NI2_HDF5GetIndxBrowserName(0)
//	if (strlen(browserName) > 0)
//		return 1
//	endif
...
//	Variable index = 0
//	do
//		browserName = NI2_HDF5GetIndxBrowserName(index)
//		if (strlen(browserName) == 0)
//			break
//		endif
//		
...
//
//Function NI2_HDF5BrowserDumpIsVisible()	// Returns true if dump window exists and is visible.
...
//End
//
//Function NI2_HDF5BrowserDumpHook(infoStr)
//	String infoStr
//
//	String event= StringByKey("EVENT",infoStr)
//
...
//End
//
//Function NI2_HDF5CreateDumpWindow()
//	DoWindow HDF5DumpNotebook
//	if (V_flag == 0)
//		Variable left, top, right, bottom
...
//// *** DISPLAY IN GRAPH ***
//
//Function NI2_HDF5BrowserGraphIsVisible()		// Returns true if dump window exists and is visible.
//										// Returns false if it does not exist or is invisible.
...
//End
//
//Function NI2_HDF5BrowserGraphHook(infoStr)
//	String infoStr
//
//	String event= StringByKey("EVENT",infoStr)
//
...
//End
//
//Function NI2_HDF5CreateBrowserGraph()
//	DoWindow HDF5BrowserGraph
//	if (V_flag == 0)
//		Variable left, top, right, bottom
...
//
//	String savedDataFolder = NI2_SetBrowserDataFolder("")					// browserWave goes in master HDF5Browser data folder
//
...
//	if (isAttribute)
//		title += NI2_SelectedAttributeName(bd, objectType==1)
//	else
//		title += NI2_SelectedDatasetName(bd)
//	endif
...
//// *** DISPLAY IN TABLE ***
//
//Function NI2_HDF5BrowserTableIsVisible()		// Returns true if dump window exists and is visible.
//										// Returns false if it does not exist or is invisible.
...
//End
//
//Function NI2_HDF5BrowserTableHook(infoStr)
//	String infoStr
//
//	String event= StringByKey("EVENT",infoStr)
//
...
//End
//
//Function NI2_HDF5CreateBrowserTable()
//	DoWindow HDF5BrowserTable
//	if (V_flag == 0)
//		Variable left, top, right, bottom
...
//	String attributeName = ""
//	if (isAttribute)
//		attributeName = NI2_SelectedAttributeName(bd, objectType==1)
//		if (strlen(attributeName) == 0)
//			return -1
//		endif
//	endif
...
//
//	STRUCT NI2_HDF5DatatypeInfo dti
//	NI2_InitHDF5DatatypeInfo(dti)					// Sets input fields.
//
...
//	endif
//
//	err = NI2_HDF5CheckDataClass(datatype_class_str, errorMessage)
//	if (err != 0)
...
//	
//	savedDataFolder = NI2_SetBrowserDataFolder("")					// browser waves go in master HDF5Browser data folder
//
//	String slabWaveStr = ""
//	ControlInfo /W=$bd.browserName UseHyperSelection
//	if (V_value)								// Use Hyperselection is checked?
...
//	String listOfWavesLoaded = ""	
//	
//	Variable needToLoadData = NI2_HDF5BrowserGraphIsVisible() || NI2_HDF5BrowserTableIsVisible()
//	if (needToLoadData)
//		String errorMessage
...
//End
//
//Function NI2_HDF5DisplaySelAttribute(bd,  isGroupAttribute)
//	STRUCT NI2HDF5BrowserData &bd
//	Variable isGroupAttribute
//	
//	if (NI2_HDF5BrowserDumpIsVisible())
...
//End
//
//Function NI2_PositionControlRelative(panelName, control, masterControl, xMode, yMode, dx, dy)
//	String panelName
...
//End
//
//Function NI2_OffsetControls(panelName, controlList, dx, dy)
//	String panelName
...
//End
//
//Function NI2_HDF5ResizeBrowser(browserName)
//	String browserName
//
...
//	
//	Variable groupListsWidth = .3 * hSpaceForLists
//	Variable datasetListsWidth = .45 * hSpaceForLists
//	Variable NikaListsWidth = .23 * hSpaceForLists
//	
...
//	NI2_PositionControlRelative(browserName, "LoadDataset", "DatasetsTitle", 1, 2, 20, 0)
////	NI2_PositionControlRelative(browserName, "AddGrouptoList", "DatasetsTitle", 1, 2, 0, -20)
//
//
//	// Set Save Waves Button
//	NI2_PositionControlRelative(browserName, "SaveWaves", "LoadDataset", 1, 2, 20, 0)
//
//	// Set Members popup menu
//	NI2_PositionControlRelative(browserName, "Members", "DatasetsTitle", 0, 2, 0, 0)
//	
...
//	NI2_OffsetControls(browserName, list, changeInDatsetsListRight, 0)
//
...
//	//Set Nika controls
//	// Set Datasets list coordinates
//	left = leftBorder + groupListsWidth + hSpaceBetweenLists+datasetListsWidth+hSpaceBetweenLists
//	top = listsTop
//
//	ListBox NikaGroupList, win=$browserName, pos={left, top}, size={NikaListsWidth, groupListsHeight}
//	// Set Load Dataset Button
//	NI2_PositionControlRelative(browserName, "NikaDataSetsTitle", "NikaGroupList", 0, 0, 30, -20)
//	Button AddGrouptoList,win=$browserName, pos={left, top-150}, size={NikaListsWidth, 20}
//	Button ClearDatasetList,win=$browserName, pos={left, top-100}, size={NikaListsWidth, 20}
//
////	NI2_PositionControlRelative(browserName, "NikaDataSetsTitle", "NikaGroupList", 0, 0, 30, -20)
//
////	Wave/O/N=0/T DataSetNames, GroupNames		//these are pointes to waht user wants to get into Igor...
...
//
//static StrConstant kPackageName = "NikaHDF5Browser"	// NOTE: Package name must be distinctive!
...
//
//Structure NI2_HDF5BrowserPrefs
...
//EndStructure
//
//Function NI2_HDF5BrowserLoadPackagePrefs(prefs)
//	STRUCT NI2_HDF5BrowserPrefs &prefs
//
//	Variable i
//
...
//		endfor
//		
//		NI2_HDF5BrowserSavePackagePrefs(prefs)		// Create default prefs file.
//	endif
//End
//
//Function NI2_HDF5BrowserSavePackagePrefs(prefs)
//	STRUCT NI2_HDF5BrowserPrefs &prefs
//
...
//
//Function NI2_HDF5CheckDataClass(dataClassStr, errorMessage)
//	String dataClassStr
//	String &errorMessage
//	
//	Variable err = 0
...
//End
//
//Function NI2_HDF5MakeHyperslabWave(path, numRows)
...
//End
//
//Constant NI2_H5S_MAX_RANK = 32
//
//Constant NI2_kHDF5DataInfoVersion = 1000		// 1000 means 1.000.
//
//Structure NI2_HDF5DataInfo					// Use with HDF5DatasetInfo and HDF5AttributeInfo functions
...
//	double dims[NI2_H5S_MAX_RANK];			// Size of each dimension.
//	double maxdims[NI2_H5S_MAX_RANK];		// Maximum size of each dimension.
//EndStructure
//
//Function NI2_InitHDF5DataInfo(di)				// Sets input fields.
//	STRUCT NI2_HDF5DataInfo &di
//	
//	// HDF5XOP uses these fields to make sure the structure passed in to it is compatible.
//	di.version = NI2_kHDF5DataInfoVersion
//	di.structName = "HDF5DataInfo"
//End
//
////	HDF5DatasetRank(locationID, name)
////	Returns rank or zero in event of error.
//Function NI2_HDF5DatasetRank(locationID, name)
//	Variable locationID
//	String name
//
//#if Exists("HDF5OpenFile")	
...
////	Returns rank or zero in event of error.
//Function NI2_HDF5AttributeRank(locationID, objectName, objectType, attributeName)
//	Variable locationID
//	String objectName
//	Variable objectType
//	String attributeName
...
//Structure  NI2_HDF5DatatypeInfo							// Use with HDF5TypeInfo functions
//	// Input fields (inputs to HDF5 XOP)
...
//EndStructure
//
//Function NI2_InitHDF5DatatypeInfo(dti)			// Sets input fields.
//	STRUCT NI2_HDF5DatatypeInfo &dti
//	
//	// HDF5XOP uses these fields to make sure the structure passed in to it is compatible.
//	dti.version = NI2_kHDF5DatatypeInfoVersion
//	dti.structName = "NI2_HDF5DatatypeInfo"
//End
//
...
//	
//	String groupPath = NI2_SelectedGroupPath(bd)	// Currently selected group
...
//			case "HDF5SaveWavesPanel":
//				Wave w = $item
//				String datasetPath = NI2_HDF5GetObjectFullPath(groupPath, NameOfWave(w))
...
//				endif
//				newGroupPath = NI2_HDF5GetObjectFullPath(groupPath, dfName)
//
...
//End
//
//Function NI2_HDF5SaveWavesPanelHook(infoStr)
//	String infoStr
//	
//	String panelName = "NI2_HDF5SaveWavesPanel"
//
//	String event= StringByKey("EVENT",infoStr)
//
//	strswitch(event)
...
//End
//
//Function NI2_HDF5SaveDataFolderPanelHook(infoStr)
//	String infoStr
//	
//	String panelName = "NI2_HDF5SaveDataFolderPanel"
//
//	String event= StringByKey("EVENT",infoStr)
//
//	strswitch(event)
...
//		SetSaveButtonState(panelName)
//		SetWindow kwTopWin,hook=NI2_HDF5SaveWavesPanelHook
//	endif
//End
//
...
//		CheckBox IncludeIgorAttributes,value=includeIgorAttributes
//		SetSaveButtonState(panelName)
//		SetWindow kwTopWin,hook=NI2_HDF5SaveDataFolderPanelHook
//	endif
//End
//
//Function NI2_SelectorNotification(SelectedItem, EventCode, panelName, controlName)
//	String SelectedItem
//	Variable EventCode
//	String panelName
//	String controlName
