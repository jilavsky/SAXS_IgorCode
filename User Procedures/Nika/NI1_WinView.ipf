...
//	endif
//	if (boxesOn)
//		DrawBoxWithTicksAtXY(pcsr(A),qcsr(A),10)
//	endif
//End
...
//	Variable i = FitOneGaussianPeak(ImageName,V_left,V_right,V_bottom,V_top)	// returns 1 if error,  0 is OK
//	if (i)
//		return i
//	endif
//
...
//		if (V_Flag==1)
//			DrawCross(x0,y0,abs(V_right-V_left),abs(V_top-V_bottom))
//		endif
//	endif
//	return 0
//End
...
//		pkList = pk
//	endif
//	if (halfWidth<1 || DimSize($ImageName,1)<1 || strlen(pkList)<5)
//		return 1
//	endif
//
...
//	Variable nx=DimSize($ImageName,0)
//	Variable ny=DimSize($ImageName,1)
//	for (i=0;i<n;i+=1)
...
//		if (FitOneGaussianPeak(ImageName,xmin,xmax,ymin,ymax))
//			x0=NaN
//			y0=NaN
//		else
//			Wave W_coef=W_coef
...
//
//
//Function FitOneGaussianPeak(ImageName,V_left,V_right,V_bottom,V_top)	// returns 1 if error,  0 is OK
//// the result is passed to calling function by W_coef or {K0, K1, ... K6}
//// this fits a 2-d gaussina to a region on an image.  The region is selected with a marquee in an image plot, or specified
...
//	if (!V_FitError)
//		Wave W_coef = W_coef
//		V_FitError += (W_coef[3]<0 || W_coef[5]<0 || W_coef[6]<0 || W_coef[6]>1) ? 32 : 0			//  constraints failed, set bit 5
//	endif
//	return V_FitError
//End
//
...
//		DoAlert 1, "Draw a cross at result"
//		if (V_Flag==1)
//			DrawCross(xMoment,yMoment,V_right-V_left,V_top-V_bottom)
//		endif
//	endif
//
...
//Function/C getZrange(image,delta)
//	Wave image
...
//	Variable lo = BinarySearchInterp(hist, delta*dh+hmin)
//	Variable hi = BinarySearchInterp(hist, (1-delta)*dh+hmin)
...
//End
//
//Function/S LoadWinViewFile(fName)
...
Function/S NI1_WinViewReadROI(fileName, i0, i1, j0, j1)
	...
	string wnote = NI1_WinViewReadHeader(fileName) // wave note to add to file read in
	if(!strlen(wnote))
		return ""
	endif
	variable xdim  = NumberByKey("xdim", wnote, "=")
	...
	string inWaveName = NI1_WinViewLoadROI(fileName, itype, xdim, i0, i1, j0, j1) // name of ROI read in
...
//
//
Function NI1_WinViewList2Header(list, header) // take list of values (from wnote) put into header
...
Function NI1_iType2fType(itype) // converts winview number type to Igor number type
	variable itype
...
Function NI1_igorType2WinView(itype) // convert Igor WaveType() to WinView number types
...
// =======================================================================
//
//Function/T NI1_reverseList(in,[sep])					// reverse the order of items in a list
...
//		String wList = NI1_reverseList(NI1_WaveListClass("speImage*","*","DIMS:2"))
...
//		x0=str2num(StringFromList(0,item,","))
//		y0=str2num(StringFromList(1,item,","))
//		DrawBoxWithTicksAtXY(x0,y0,wid)
//	endfor
//	return 1
//End
