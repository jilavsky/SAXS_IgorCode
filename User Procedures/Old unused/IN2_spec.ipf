...
Function DisplaySpecScan(scanNum,overlay)
	...
	xname = StrVarOrDefault("specXwave",xname)
	yname = StrVarOrDefault("specYwave",yname)
	...
	String OrigFile = StringByKey("DATAFILE",note($xname),"=")
	...
	if (cmpstr(xname,"ttheta")==0 || cmpstr(xname,"Two_Theta")==0)
...
Function specReadFromSline(fileVar,FilePosScan)
	...
	if (stringmatch(specXwave,"H"))			// an hkl, scan pick pick h,k, or l that changes the most
		Variable dhkl, dtemp
		Wave hkl=H
...
Function AddStandardWaveScaling(nameList)
	...
	unitList += "Two_Theta:¡;Theta:¡;Chi:¡;Phi:¡;anal-2th:¡;anal-th"
	String name
	Variable i = 0
	do
