...
Function/S IN4_ProcessSelectedData(FileNameList)		
	...
	//Force recalculation. Using Python or Igor. 
	if(Recalculate)
		if(UsePython)
#if(IgorVersion()>9.99)
			...
			if(IncludeImage)
				print "Need to finish importing images from Python in Matilda support" 
			endif
			//and now, let's process the list of the files. 
			for(i=0;i<itemsInList(FileNameList);i+=1)
			
				if(IncludeUSAXS)
					PathToFileWithData = IN4_FIndProperPathToData(PathToData,"usaxs") 
					//set 1 - deal with USAXS data
					PythonPath = ParseFilePath(5, PathToFileWithData, "\\", 0, 0)
				...
				if(IncludeSAXS)
					PathToFileWithData = IN4_FIndProperPathToData(PathToData,"saxs") 
					//set 1 - deal with SAXS data
					PythonPath = ParseFilePath(5, PathToFileWithData, "\\", 0, 0)
					PythonPath = RemoveEnding(PythonPath)
					PythonPath = ReplaceString("\\", PythonPath, "/")			
					PythonSampleFile = "[['"+PythonPath+"','"+SampleName+"'],]"
				...
				if(IncludeWAXS)
					//now WAXS
					PathToFileWithData = IN4_FIndProperPathToData(PathToData,"waxs") 
					//set 1 - deal with SAXS data
					PythonPath = ParseFilePath(5, PathToFileWithData, "\\", 0, 0)
					PythonPath = RemoveEnding(PythonPath)
					PythonPath = ReplaceString("\\", PythonPath, "/")							
		...
		//next is recalculating data using Igor Pro

				//USAXS , if user wants it
				if(IncludeUSAXS)
					for(i=0;i<itemsInList(FileNameList);i+=1)
						SampleName = StringFromList(i,FileNameList)
				...
				if(IncludeImage)
					for(i=0;i<itemsInList(FileNameList);i+=1)
						SampleName = StringFromList(i,FileNameList)
						returnedFilePathName = IN4_ImportImage(SampleName)
						ImportedFolders += returnedFilePathName+";"
					endfor			
				endif
				//now SAXS, if user wants it. 
				if(IncludeSAXS)
					...
					pathToData = IN4_FindPathStringToRealData("saxs")
					IgortemSampleName = ReplaceString(".h5", SampleName, ".hdf")
					IgortemSampleName = IN4_FixDataFileName(IgortemSampleName, pathToData, ".hdf")	//fix not matching parts of names... 
				...
				if(IncludeWAXS)
 					//init the SAXS 
					SampleName = StringFromList(0,FileNameList)
					pathToData = IN4_FindPathStringToRealData("waxs")
					IgortemSampleName = ReplaceString(".h5", SampleName, ".hdf")
					IgortemSampleName = IN4_FixDataFileName(IgortemSampleName, pathToData, ".hdf")	//fix not matching parts of names... 
	...
	//ONLY Import the data from list of files, do NOT recalculate. 
		if(UsePython)
#if(IgorVersion()>9.99)
			...
			Abort "You need Igor Pro 10 and Python environment with Matilda in order to use Python to reduce data"
#endif		
		 else // USING IGOR CODE, imports selected of USAXS, SAXS, and WAXS		
			for(i=0;i<itemsInList(FileNameList);i+=1)
...
Function/S IN4_SetupPathAndGetListOfFiles()
	//ask user for path to data
	//find path for segment (USAXS, SAXS, WAXS) has more files not to miss any
	//call this Indra4DataPath and generate list of proper files (h5 or hdf)
	string FilenameList = ""
	
	NewPath/M="Locate Sample folder"/O/Q/Z Indra4DataPath
	if(V_Flag != 0)
		return ""
	endif
	PathInfo Indra4DataPath	
	string PathStr = S_Path
	string LastPathStr=StringFromList(ItemsInList(PathStr,":")-1, PathStr , ":")
	string tempFldrList, tempSubfolderStr, usaxsFldrStr, tempStrU, tempStrS, tempStrW
	string saxsFldrStr, waxsFldrStr, AgBNameStr
	variable usaxsFiles, saxsFiles, waxsFiles, maxCountFiles
	string FinalFileList
	//OK, now we need to figure out, where are we...
	//option 1, user located sample folder, default data (or any other user name). There are 3 subfolder here. 
	//option 2, user found one of the subfolder with segment data
	
	if(!(GrepString(LastPathStr, "_usaxs$")||GrepString(LastPathStr, "_.axs$")))	//this should make sure we do not have segment folder
		//now we need to check if at least one segment folder is here...
		tempFldrList = IndexedDir(Indra4DataPath, -1, 0)				//This returns files: IndexedFile(tempPath, -1, extension)
		// print tempFldrList should return something like this:   data_saxs;data_waxs;data_usaxs;
		//some may be missing, so at least one !
		// if this is the case, then we need to evaluate which oine has more files of appropriate type there...
		if(GrepString(tempFldrList, "_usaxs")||GrepString(tempFldrList, "_saxs")||GrepString(tempFldrList, "_waxs"))
			//first check if usaxs path exists:
			usaxsFldrStr = removeending(grepList(tempFldrList,"_usaxs"),";")
			saxsFldrStr = removeending(grepList(tempFldrList,"_saxs"),";")
			waxsFldrStr = removeending(grepList(tempFldrList,"_waxs"),";")
			if(strlen(usaxsFldrStr)>0)				//exists
				newPath/O/Q tempPath, PathStr+usaxsFldrStr+":"
				tempStrU = IndexedFile(tempPath, -1, ".h5")
				usaxsFiles = ItemsInList(tempStrU)
			endif
			if(strlen(saxsFldrStr)>0)			//exists
				newPath/O/Q tempPath, PathStr+saxsFldrStr+":"
				tempStrS = IndexedFile(tempPath, -1, ".hdf")
				AgBNameStr = removeending(grepList(tempStrS,"AgBehenateLaB6"),";")
				tempStrS = removeFromList(AgBNameStr, tempStrS)
				saxsFiles = ItemsInList(tempStrS)
			endif
			if(strlen(waxsFldrStr)>0)			//exists
				newPath/O/Q tempPath, PathStr+waxsFldrStr+":"
				tempStrW = IndexedFile(tempPath, -1, ".hdf")
				AgBNameStr = removeending(grepList(tempStrW,"AgBehenateLaB6"),";")
				tempStrW = removeFromList(AgBNameStr, tempStrW)
				waxsFiles = ItemsInList(tempStrW)
			endif
			if(usaxsFiles+saxsFiles+waxsFiles<1)
				abort "No data folders found"
				return ""			
			endif
			//now need to figure out if to use USAXS or SAXS or WAXS
			//assume normally USAXS is the main choice
			FinalFileList = tempStrU
			maxCountFiles = usaxsFiles
			newPath/O/Q Indra4DataPath, PathStr+usaxsFldrStr+":"
			if(saxsFiles > maxCountFiles)	//SAXS has more data
				FinalFileList = tempStrS
				maxCountFiles = saxsFiles
				newPath/O/Q Indra4DataPath, PathStr+saxsFldrStr+":"
			endif
			if(waxsFiles > maxCountFiles)	//WAXS has even more data
				FinalFileList = tempStrW
				maxCountFiles = waxsFiles
				newPath/O/Q Indra4DataPath, PathStr+waxsFldrStr+":"
			endif
			FilenameList = FinalFileList
		else	//not appropriate sample folder
			
			Abort "This is not sample folder with _usaxs, _saxs, or _waxs folders inside"
			
		endif
	else	//we are in one of the segment folders. Let's use this one for list of samples. 
		//OK so lets use local content for the list, user has reasons to do so...
		//in this case  Indra4DataPath is set correctly
		if(GrepString(LastPathStr, "_usaxs$"))		//USAXS folder
			tempStrU = IndexedFile(Indra4DataPath, -1, ".h5")
			FilenameList = tempStrU
		
		elseif(GrepString(LastPathStr, "_.axs$"))	//SAXS or WAXS folder
			tempStrS = IndexedFile(Indra4DataPath, -1, ".hdf")
			AgBNameStr = removeending(grepList(tempStrS,"AgBehenateLaB6"),";")
			tempStrS = removeFromList(AgBNameStr, tempStrS)
			FilenameList = tempStrS
		else
			FilenameList = ""
		endif
...
Function IN4_SaveSWAXSDataInNexus(DataPathStr, SAXSorWAXS, filenameInput)
	...
	if(SAXSorWAXS==1)	//SAXS
		tempPathStr =  IN4_FIndProperPathToData(DataPathStr,"saxs")
	elseif(SAXSorWAXS==2)	//WAXS
		tempPathStr =IN4_FIndProperPathToData(DataPathStr,"waxs")
	else
		return 0
	endif
	
	newPath/O/C/Q SWAXSPath, tempPathStr  
...
Function IN4_initProcessSWAXSdata(fileName, isSAXS)
	...
	string SWAXSPathStr
	if(isSAXS)
	    SWAXSPathStr= IN4_FIndProperPathToData(PathToData,"saxs")
	else
	    SWAXSPathStr= IN4_FIndProperPathToData(PathToData,"waxs")
	endif
	//now start Nika
	...
	//Nika parameters to overwrite here
	...
	//these are from IN4 GUI overwrite parameters
	NVAR SAXSNumPoints			= root:Packages:Indra4:SAXSNumPoints
	NVAR SAXSMaxNumberOfPoints 	= root:Packages:Indra4:SAXSMaxNumberOfPoints
	NVAR SAXSFixBackgOversub = root:Packages:Indra4:SAXSFixBackgOversub
	if(isSAXS)
		USAXSSAXSselector=1
		USAXSWAXSselector = 0
		QvectorNumberPoints = SAXSNumPoints
		QvectorMaxNumPnts = SAXSMaxNumberOfPoints
		if(QvectorMaxNumPnts)
			QbinningLogarithmic = 0
		else
			QbinningLogarithmic = 1
		endif	
		FixBackgroundOversubtraction = SAXSFixBackgOversub
	else
		USAXSSAXSselector=0
		USAXSWAXSselector = 1
		QvectorMaxNumPnts = 1
...
static Function/S IN4_ImportDataSet(FileName)
	string FileName 	
	//assuminng this is USAXS data set!
	...
	string RealSampleName, returnedFilePathName
	string ImportedFolders =""
	string UsaxsFolderStr=""
	string actualExtension="h5"
	if(grepstring(FileName,"\.hdf"))
		actualExtension = "hdf"
	endif
	...
	if(IncludeUSAXS)
		...
		string loadedFile, loadedFileShort
	endif
	if(IncludeImage)
		SampleName = FileName
		returnedFilePathName = IN4_ImportImage(SampleName)
	...
	if(IncludeSAXS)
		...
		loadedFile = ReadMyNXcanSASSWAXS(PathToFileWithData, 1, SampleName)
	...
	if(IncludeWAXS)
		...
		loadedFile = ReadMyNXcanSASSWAXS(PathToFileWithData, 2, SampleName)		
		if(strlen(loadedFile)>0)
			//for sequences, the file loaded may not be the same as we asked for, so we need to fix things... 
			if(!stringmatch(loadedFile,SampleName))
				loadedFileShort = removeending(loadedFile,".hdf")
...
static FUnction/S IN4_FixDataFileName(fileNameIn, PathToDataStr, extension)
	string fileNameIn, PathToDataStr, extension

	//file name in is name...
	// PathToDataStr is strong to data path
	//extension if "h5" or "hdf". 

	// unluckily, in case we have series in time or temperature, file names of filess matching together are not the same. 
	// need smarter lookup. These files have same all parts, except part with _87min or _87degC. 
	// need to check if the file exists and fi yes, do notrmal processing.
	// if file does not exist, we need to try to find file which ends with the same number and check that anything except part with min or degC are the same.
	newPath/O/Q/Z tempPath, PathToDataStr
	if(V_Flag!=0)
		return ""
	endif
	...
	Open/R/P=tempPath/Z tempfileID as fileNameIn	// test if it will open as a regular file
	if(tempfileID>0)	//exists
		close tempfileID
		fileName = fileNameIn
		//and keep going
	else
		//need to figrue more out.
		string allfiles=IndexedFile(tempPath, -1, extension) 
		//print allfiles
		//now find files with same order number 
		string orderNumberS = StringFromList(ItemsInList(fileNameIn,"_")-1,fileNameIn,"_")
		//print orderNumberS
		string Grepexprt="_"+orderNumberS+"$"
		string listOfFiles = GrepList(allfiles, Grepexprt)
		string selectedFiletmp
		//print listOfFiles
		if (ItemsInList(listOfFiles)>1)
			//check the start of the file seems to be the same. 
			string StartNameS = StringFromList(0,fileNameIn,"_")
			Grepexprt="^"+StartNameS
			listOfFiles = GrepList(listOfFiles, Grepexprt)
			if(ItemsInList(listOfFiles)>1)	//give up, pick first
				selectedFiletmp = stringFromList(0,listOfFiles)
				print "Could not properly identify matching USAXS/SAXS/WAXS file, poor naming choice, loading file:"+selectedFiletmp
			else
				selectedFiletmp = listOfFiles
			endif
		else
			selectedFiletmp = listOfFiles		
		endif
		fileName = removeending(selectedFiletmp,";")		
	endif
...
static Function/S IN4_FindPathStringToRealData(whichData)
		string whichData
		...
		strswitch (whichData)
			case "usaxs" :
			    SWAXSPathStr= IN4_FIndProperPathToData(PathToData,"usaxs")
			    break
			case "saxs":
			    SWAXSPathStr= IN4_FIndProperPathToData(PathToData,"saxs")
			    break
...
//**********************************************************************************************************
//**********************************************************************************************************
static Function/S IN4_FindProperPathToData(anyPathStr,DataType)
	string anyPathStr,DataType
	//returns path modified by changing any _saxs, _usaxs, _waxs into type we need
	// types are usaxs, saxs, waxs as strings.
	
	string newPathstr
	if(grepString(anyPathStr,"_saxs"))
		newPathStr =  ReplaceString("_saxs", anyPathStr, "_"+DataType)
	elseif(grepString(anyPathStr,"_usaxs"))
		newPathStr =  ReplaceString("_usaxs", anyPathStr, "_"+DataType)
	elseif(grepString(anyPathStr,"_waxs"))
		newPathStr =  ReplaceString("_waxs", anyPathStr, "_"+DataType)
	else
		abort "Bad call in IN4_FindProperPathToData"
	endif
...
//**********************************************************************************************************
//**********************************************************************************************************
static Function IN4_FixParamsAfterImport(UsaxsFolderStr)
	string UsaxsFolderStr
	
	
	if(strlen(UsaxsFolderStr)>0)
		//here we tweak various parameters after we import the data
...
Function/S ReadMyNXcanSASSWAXS(DataPathStr, SAXSorWAXS, filenameInput)
    String filenameInput, DataPathStr
    variable SAXSorWAXS 
	//SAXSorWAXS = 1 for SAXS, 2=waxs
	...
	variable FileID, groupID, tempfileID
	string AttrString, fileNameLocal
	string tempPathStr 
	if(SAXSorWAXS==1)	//SAXS
		tempPathStr =IN4_FindPathStringToRealData("saxs")
	elseif(SAXSorWAXS==2)	//WAXS
		tempPathStr = IN4_FindPathStringToRealData("waxs")
	else
		return ""
	endif
	fileNameLocal = IN4_FixDataFileName(filenameInput, tempPathStr,  ".hdf") //also sets tempPath... 
	if(strlen(fileNameLocal)<4)
		return ""		//no file found, get out. 
	endif
	

	HDF5OpenFile /P=tempPath /R /Z fileID as fileNameLocal
	if(V_Flag!=0)	//failed to open, bail out.
		return ""
	endif
	...
	if(strlen(fileAttribs)<1)
		Print "Data do not exist for file:"+filenameInput +", Matilda data reduction failed or may not be done yet"
		return ""
	endif
	...
	if(!(stringmatch(file_instrument,"12IDE USAXS")||stringmatch(file_instrument,"APS USAXS at 12-ID-E")))
		Debugger
		print "These data are not from correct instrument"
		return ""
	endif
	if(!stringmatch(file_creator,"Matilda NeXus writer"))
		Debugger
		Print "Data for "+filenameInput +" are not prepared by Matilda"
		return "" 
	endif
...
static Function IN4_LoadWaveAndAppendAttribs(locID, IgorWaveName, H5DataName)
		variable locID
...
static Function IN4_AppendAttributesInWaveNote(locID, IgorWavename, H5DataName)
		variable locID
		String IgorWavename, H5DataName
...
//**********************************************************************************************************
static Function/S IN4_ImportImage(filenameInput)
    String filenameInput
	//this function will import the image file, if it exists and place it in Igor. 
	//assume for now these are jpg images, in the future this may be image in dhdf5 Nexus file. 
	//depends on Matilda support. 
	//jpg file is located in USAXS location aand this is called after USAXS data reduction run. 
	DFREF oldDf = GetDataFolderDFR()
	
	//create the location:
	NewDataFolder/O/S root:Images
	string newFolderName = StringFromList(0, filenameInput, ".")	
	string ImageName= ReplaceString("h5",filenameInput,"jpg") 	//this is if called from USAXS folder
	ImageName= ReplaceString("hdf",ImageName,"jpg")			//and this is if called from SAXS/WAXS folders. 
	print "Saving Image to new folder root:Images:" + newFolderName
	NewDataFolder/O/S $("root:Images:"+newFolderName)
	...
	if(V_flag==1)
		...
		print "Image load for "+newFolderName+" failed"
	endif	
	SetDataFolder oldDf
...
static Function IN4_ImportRawUSAXS(filenameInput)	//this reads RAW USAXS data,  
    String filenameInput
	...
	string PathToFileWithData = IN4_FindPathStringToRealData("usaxs") 
	filenameInput =  replaceString(".hdf",filenameInput,".h5")
	filenameInput = IN4_FixDataFileName(filenameInput, PathToFileWithData, ".h5") //also sets tempPath... 
	
	HDF5OpenFile /P=tempPath /R /Z fileID as filenameInput
	if(V_Flag!=0)
	...
	if(StringMatch(scan_mode, "USAXS fly scan" )&&(StringMatch(instrumentName, "APS 12-ID-E USAXS")||StringMatch(instrumentName, "12IDE USAXS")))
...
static Function ReadMyNXcanSASUSAXS(filenameInput)
    String filenameInput
	...
	string PathToFileWithData = IN4_FindPathStringToRealData("usaxs") 
	fileNameLocal =  ReplaceString(".hdf",filenameInput,".h5")
	fileNameLocal = IN4_FixDataFileName(fileNameLocal, PathToFileWithData, ".h5") //also sets tempPath... 
	if(strlen(fileNameLocal)<4)//no matching file, bail out. 
		return 0
	endif	
	//some file shoudl exist... 
	HDF5OpenFile /P=tempPath /R /Z fileID as fileNameLocal
	if(V_Flag!=0)	//failed to open, bail out.
		return 0
	endif
	...
	if(!(stringmatch(file_instrument,"12IDE USAXS")||stringmatch(file_instrument,"APS USAXS at 12-ID-E")))
		Debugger
		Print "Data for "+filenameInput+" are not from correct instrument"
		return 0
	endif
	if(stringmatch(file_creator,"Matilda NeXus writer")==0 && stringmatch(file_creator,"Indra Nexus writer")==0 && stringmatch(file_creator,"*saveFlyData.py")==0)
		Debugger 
		print "Data for "+filenameInput+" are not prepared by Matilda"
		return 0
	endif
...
static Function IN4_SaveUSAXSDataInNexus(filenameInput)
    String filenameInput
...
static Function IN4_NXwriteGrpAttribList(locID,NXlocation, AttrList)
	variable locID
...
static Function/S IN4_FindGroupsWithAttributes(locID, pathToLoc, AttributesList)
			variable locID
...
static Function/S IN4_ReadAttributesInDict(locID,ItemNamePath, isGroup)
...
//**********************************************************************************************************
//**********************************************************************************************************
static Function/S IN4_ReadGroupItemsToDict(locID,PathToGroup,ListOfItemsToKeep)
	variable locID
...
//**********************************************************************************************************
//**********************************************************************************************************
static Function IN4_ReadVariable(locID,PathToItem)
	variable locID
	string PathToItem
	variable result
